...
export default function BannerPage() {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        ...
        const response = await fetch(`/api/users/${userId}/banners`);
  ...
  const handleDelete = async (banner: Banner) => {
    if (confirm(`Are you sure you want to delete ${banner.title}?`)) {
      try {
        const response = await fetch(`/api/users/${userId}/banners/${banner.id}`, {
          method: "DELETE",
        });
        ...
        setBanners(banners.filter(p => p.id !== banner.id));
      } catch (error: unknown) {
  ...
  const handleFormSubmit = async (bannerData: Omit<Banner, "id">) => {
    try {
      setError(null);

      if (editingBanner) {
        // Fix: Menggunakan ID produk langsung dari objek editingProduct
        const response = await fetch(`/api/users/${userId}/banners/${editingBanner.id}`, {
          method: "PATCH",
          headers: {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg mb-6 border border-blue-100">
        <h2 className="font-medium text-blue-800 mb-2">Banner Management</h2>
        <p className="text-sm text-blue-700">
          Create and manage Banner that appear on your website. 
      ...
      {banners.length === 0 ? (
        <div className="text-center py-10 bg-gray-50 rounded-lg">
          <p className="text-gray-500">No banners found. Click Add Banners to create one.</p>
        </div>
      ) : (
        <BannerTable 
          banners={banners} 
          ...
          <div className="bg-white rounded-lg w-full max-w-md shadow-xl">
            ...
            <div className="p-4">
              ...
              <div className="mb-4">
                ...
                <div className="p-2 bg-gray-50 rounded border text-center font-medium">{selectedBanner.title}</div>
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                <div className="p-3 bg-gray-50 rounded border">{selectedBanner.description}</div>
              </div>
